#### Let's try it again at timepoint 24h
...
targets3 <- read_tsv("ca_study.txt")# read in your study design
path3 <- file.path(targets3$sample, "abundance.tsv") # set file paths to your mapped data
...
group3 <- factor(targets3$group)  # with this function we want to encode the vector group2 of our dataframe(tibble) targets2 as a factor
design3 <- model.matrix(~0 + group3)  # we are using the new factor group2 to expand it to a matrix according to the appearance of disease and healty in the 8 samples. It is then annotated as 1.
colnames(design3) <- levels(group3) # the column names are healthy and disease and we specifiy them know as levels

v.DEGList3.filtered.norm <- voom(myDGEList3.filtered.norm, design3, plot = T) # we now add the counts saved as filtered.norm to the design2, as well as the weights and transcripts of the sample 
fit3 <- lmFit(v.DEGList3.filtered.norm, design3) # now we fit the arrays by weighted or generalized least squares -> so disease and healthy
contrast.matrix3 <- makeContrasts(infection = disease - healthy,      # here we specify the contrasts between the disease and healthy -> disease is 1 and healthy is -1
                                  levels=design3)                      # and we also create a contrast matrix

fits3 <- contrasts.fit(fit3, contrast.matrix3)                        # here we want to compute estimated coefficients and standard errors for given set of contrasts. The input is the lmfit and contrast matrix. Finally, it re-orientated the fitted model object from the coefficients to 'disease' and 'healthy'.
ebFit3 <- eBayes(fits3)                                               # we know compute moderated t-statistics, F-statistic and log-odds of our fitted data
myTopHits3 <- topTable(ebFit3, adjust ="BH", coef=1, number=40000, sort.by="logFC") # with this we extract the top-ranked genes from our linear model fit object and sort them by the (absolute) coefficient representing the log-fold-change
...
gt(myTopHits.df3) # here we can see our t-statistics; B-statistics and the p-values
...
ggplotly(vplot3)                # So the blue square is giving us the own regulated differentially expressed genes and the red square the up regulated ones. For out p adjusted values we have pretty normal differentially expressed genes. Apparently none of our transcripts are significant.


results3 <- decideTests(ebFit3, method="global", adjust.method="BH", p.value = 0.05, lfc = 1)     # checking which genes are significantly differentially expressed for each contrast from our fitted object. We don't have any significant differentially expressed genes. The p.value here represents the alpha significance; which normally is 0.01 in medical statistics. 
colnames(v.DEGList3.filtered.norm$E) <- sampleLabels3       #specifing sampleLables according to pathogen1 to control4
diffGenes3 <- v.DEGList3.filtered.norm$E[results3[,1] !=0,] #taking from v.DEGList2.filtered.norm the table E and only the results from the first row and only the ones which unequal 0
diffGenes.df3 <- as_tibble(diffGenes3, rownames = "geneID")  # now we put diffGenes2 into a tibble
...
gost.res3 <- gost(rownames(myTopHits3), organism = "tccriollo", correction_method = "fdr", significant = F)
gost.res3
# produce an interactive manhattan plot of enriched GO terms
gostplot(gost.res3, interactive = T, capped = F) #set interactive=FALSE to get plot for publications
